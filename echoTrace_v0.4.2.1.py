...
class SoundSourceLocalization3D(QMainWindow):
    def __init__(self):
        ...
        # Binalar
        self.buildings = self.generate_random_buildings(random.randint(1, 3))
    ...
    def generate_random_buildings(self, count):
        ...
        for _ in range(count):
            ...
            z = 0  # Binaların tabanı z=0 düzleminde olsun
            building = {
    ...
    def draw_building(self, building):
        """Verilen binayı grafiğe ekler."""
        x, y, z = building['position']
        dx = building['width']
        dy = building['depth']
        dz = building['height']
        # Bina için bir küp ekle
        self.ax.bar3d(x, y, z, dx, dy, dz, color='brown', alpha=0.5, shade=True)
    ...
    def initial_plot(self):
        ...
        xlim = [-15, 25]
        ylim = [-15, 25]
        zlim = [-10, 30]  # Z limiti binaların yüksekliğine göre artırıldı
    ...
    def randomize_positions(self):
        """Mikrofon ve gürültü kaynaklarının pozisyonlarını ve binaları rastgele olarak değiştirir."""
    ...
    def update_plot_elements(self):
        """Grafik öğelerini günceller (mikrofonlar, gürültü kaynakları, ses kaynakları, binalar)."""
        self.ax.clear()
        self.initial_plot()
        # Fare ile döndürme ayarları (sağ tık ile döndürme)
        self.ax.mouse_init(rotate_btn=3, zoom_btn=None)
