...
class SoundSourceLocalization(QMainWindow):
    ...
    def calculate_db(self, distance):
        """Verilen mesafeye göre desibel değerini hesaplar."""
        # Desibel hesaplama: dB = 100 - 20 * log10(r)
        # r = mesafe (m)
        # 1 m'de 100 dB, 10 m'de 80 dB gibi
        if distance <= 0:
            return 100  # Maksimum desibel
        db = 100 - 20 * math.log10(distance)
        return db

    def compute_average_db(self, source_pos):
        """Tüm mikrofonlardan gelen desibel değerlerinin ortalamasını hesaplar."""
        db_values = []
        for mic in self.mic_positions:
            distance = self.calculate_distance(mic, source_pos)
            db = self.calculate_db(distance)
            db_values.append(db)
        average_db = np.mean(db_values)
        return average_db
    ...
    def on_click(self, event):
        if event.inaxes == self.ax:
            if event.button == 1:
                ...
                self.average_db = self.compute_average_db(self.source_point)
    ...
    def update_plot(self):
        ...
        if self.source_point is not None:
            ...
            self.ax.text(self.source_point[0], self.source_point[1], f'  {self.average_db:.2f} dB', color='red', fontsize=12, ha='left', va='center')
