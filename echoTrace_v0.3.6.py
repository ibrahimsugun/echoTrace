    ...
    def on_click(self, event):
        if event.inaxes == self.ax:
            if event.button == 1:
                ...
                time_stamps = []
                for mic in self.mic_positions:
                    # Her kaynağın zaman damgasını hesapla ve ortalama al
                    t_total = 0
                    for source in sources:
                        dist = self.calculate_distance(mic, source['position'])
                        t = dist / SOUND_SPEED
                        t_total += t
                    t_average = t_total / len(sources)  # Ortalama zaman damgası
                    time_stamps.append(t_average)
