    ...
    def perform_localization(self):
        ...
        for idx_mic, mic in enumerate(self.mic_positions):
            ...
            for source in source_points:
                ...
                blocked = self.is_path_blocked(source['position'], mic)
                if blocked and source['position'] is self.source_point:
                ...
                power = 10 ** (db / 10)
                total_power += power
                self.calculation_steps += f"  Kaynak ({source['position'][0]:.2f}, {source['position'][1]:.2f}, {source['position'][2]:.2f}), Mesafe: {distance:.2f} m, dB: {db:.2f}, Engellenmiş: {'Evet' if blocked else 'Hayır'}\n"
        ...
        def objective_function(pos):
            ...
            for i, mic in enumerate(self.mic_positions):
                blocked = self.is_path_blocked(pos, mic)
                if blocked:
                    total_power = 0  # Engellenmişse kaynak katkısı yok
                else:
